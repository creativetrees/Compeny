...
{{--
    The mobile menu panel is a SIBLING of <header>, not a descendant — nesting a
    `fixed inset-0` element inside another `fixed` ancestor makes Chromium miscompute
    its height against the ancestor's own (content-sized) box once the page has a
    scroll offset, collapsing the panel to the header's height instead of the viewport.
--}}
<div x-data="siteHeader" @scroll.window="onScroll()" @resize.window.debounce="onResize()">
<header
    :class="headerClass"
